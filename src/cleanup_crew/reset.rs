...
pub async fn run(mut context: AppContext, request: ResetRequest) -> Result<()> {
    ...
    let target_state = if let Some(playable) = target_scenario_config.playable_state {
        // ensure the format is correct with ScenarioExportType::Type.bin
        if !playable.to_lowercase().ends_with(".bin") {
            println!(
                "Playable state {} in scenario config is not in the correct format (should end with .bin)",
                playable
            );
            return Ok(());
        }
    ...
    {
        let state = &mut context.scenario_state;
        state.active_scenario = Some(ScenarioRef {
            scenario: target_name.clone(),
            state_file: target_scenario_file.to_string_lossy().to_string(),
        });
        state.save(&config.paths.scenario_state_file)?;
    }
