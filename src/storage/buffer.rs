...
pub use super::iter::OxicodeIterator;
...
pub struct OxicodeObjectBuffer<T> {
    ...
    /// Intermediate buffer for encoding objects to oxicode before writing to disk
    encode_buffer: Vec<u8>,
}

impl<T> OxicodeObjectBuffer<T> {
    pub fn new(file_path: impl AsRef<Path>) -> Result<Self, Box<dyn Error>> {
        Ok(OxicodeObjectBuffer {
...
impl<T: oxicode::Decode<()>> OxicodeObjectBuffer<T> {
    pub fn into_reader(
        self,
    ) -> Result<OxicodeIterator<T, BufReader<std::fs::File>>, Box<dyn Error>> {
        let mut inner = self.writer.into_inner()?;
        inner.0.seek(std::io::SeekFrom::Start(0))?;
        Ok(OxicodeIterator::from_file(inner.0))
    }
}

impl<T> Storage<T> for OxicodeObjectBuffer<T>
where
    T: oxicode::Encode,
{
    #[inline]
